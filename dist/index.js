...
server.tool("verify_coverage", "Analyze whether a claim is covered under a given policy. Checks insuring agreements, exclusions, conditions, and endorsements. Returns coverage determination with confidence score.", {
...
server.tool("check_exclusions", "Identify all applicable exclusions for a claim scenario. Returns detailed analysis of each standard ISO and manuscript exclusion with applicability assessment, exceptions, and buyback availability.", {
...
// ── Tool: analyze_limits ────────────────────────────────────────────
server.tool("analyze_limits", "Calculate available limits after prior claims, aggregate erosion, deductible application, and SIR satisfaction. Shows sublimit analysis and exhaustion risk.", {
    per_occurrence_limit: z.number().describe("Per-occurrence or per-claim limit"),
    aggregate_limit: z.number().describe("General aggregate limit"),
    ...
    else if (projectedTotal >= aggregate_limit)
        exhaustionRisk = "high";
    else if (projectedTotal >= aggregate_limit * 0.75)
        exhaustionRisk = "moderate";
    else if (projectedTotal >= aggregate_limit * 0.5)
        exhaustionRisk = "low";
    const analysis = {
...
server.tool("find_coverage_gaps", "Identify uninsured or underinsured exposures by comparing a risk profile against a policy portfolio. Returns prioritized gaps with recommended coverage and estimated premiums.", {
    ...
    for (const risk of riskProfile) {
        ...
        const hasCoverage = existingTypes.some(t => coverageKeywords.some(k => k.length > 3 && t.includes(k)));
        if (!hasCoverage) {
            // Estimate minimum limit based on revenue
            let recommendedLimit = risk.minimumLimit;
            if (annual_revenue > 10_000_000)
                recommendedLimit = Math.max(recommendedLimit, risk.minimumLimit * 2);
            if (annual_revenue > 50_000_000)
                recommendedLimit = Math.max(recommendedLimit, risk.minimumLimit * 3);
            ...
            const matchingPolicy = existing_policies.find(p => coverageKeywords.some(k => k.length > 3 && p.type.toLowerCase().includes(k)));
...
server.tool("coordinate_policies", "Determine priority and contribution across multiple applicable policies for a single claim. Analyzes other-insurance clauses, attachment points, and contribution methods.", {
...
server.tool("verify_additional_insured", "Check additional insured status, endorsement types, and coverage scope. Determines whether an entity qualifies as an additional insured under the policy and what coverage applies.", {
...
// ── Helper Functions ─────────────────────────────────────────────────
function checkCglExclusionTrigger(letter, lossLower) {
    const triggers = {
...
function checkPropertyExclusionTrigger(exclusionName, lossLower) {
    const triggers = {
